...
class MockIntegrator:
    ...
    def __init__(self, nsteps=0, temp=None, temperature=None, _kT=None):
        self.nsteps = nsteps
        # Langevin style
        if temp is not None:
            self.temp = temp
...
class TestASEBackendGetTargetTemperature:
    ...
    def test_nose_hoover_integrator_kT(self):
        """Should read _kT from Nose-Hoover-style integrator and convert from eV to Kelvin."""
        atoms = MockAtoms()
        calc = MockGADESCalculator(MockBaseCalculator())
        backend = ASEBackend(calc, atoms)

        # Real ASE Nose-Hoover stores self._kT = units.kB * temperature_K (in eV)
        backend.integrator = MockIntegrator(_kT=500.0 * ase_units.kB)

        assert backend._get_target_temperature() == pytest.approx(500.0)
