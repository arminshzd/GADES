...
adp_atom_indices = jnp.array([atom.index for atom in traj.topology.atoms if atom.residue.name != 'HOH']) # what atoms do you want to pick
...
# box = jnp.array([[23.648, 0.0, 0.0], [0.0, 23.648, 0.0], [0.0, 0.0, 23.648]])
box = jnp.array([jnp.array(system.getDefaultPeriodicBoxVectors()[x].value_in_unit(unit.angstrom)) for x in [0,1,2]])
nbList = NeighborListFreud(box, 3, potentials.meta["cov_map"])
nbList.allocate(positions_all)
pairs = nbList.pairs

solvent_indices = jnp.array([i for i in range(positions_all.shape[0]) if i not in adp_atom_indices]) # this might not be solven coordinates for all systems
...
def update_biased_forces(sim, biased_force, adp_atom_indices):
    ...
    positions_adp = jnp.array(positions)[adp_atom_indices]
    ...
    for i in range(sim.system.getNumParticles()):
        if i in adp_atom_indices:
            idx = list(adp_atom_indices).index(i)
...
simulation.reporters.append(StateDataReporter("scalars.csv", 100, time=True, potentialEnergy=True, temperature=True))
